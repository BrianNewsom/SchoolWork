...
Name: Brian Newsom
SID: 830372483
Description: A program designed to encode or decode messages using a cipher with a keyword. NOW INCLUDES NUMBERS 0-9
*/
...
int main()
{
	...
	while( menu_selection != 3 )
	{
		...
		if( menu_selection != 3 )
		{
			...
			while( alpha_shift > 35 || alpha_shift < 0 )
			{
			...
			if( menu_selection == 1 )
			{
				...
				for(unsigned int i=0 ; i<36 ; i++)
					cipher_alphabet.insert(pair<char, char>(alphabet[i],keyword[i]));
			...
			if( menu_selection == 2 )
			{
				...
				for(unsigned int i=0 ; i<36 ; i++)
					cipher_alphabet.insert(pair<char, char>(keyword[i],alphabet[i]));
...
string removeNonLetters( string text )
{
	...
	for( s_iter = text.begin(); s_iter != text.end(); s_iter++ )
	{
		...
		if(( *s_iter < 'A' || *s_iter > 'Z' )&&( *s_iter < 'a' || *s_iter > 'z' )&&( *s_iter < '0' || *s_iter >'9'))
		{

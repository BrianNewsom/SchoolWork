...
int main( )
{
	...
	{
		list_iter=find(alphabet.begin(), alphabet.end(), keyword[i]);
		if(list_iter!=alphabet.end())
		{
			alphabet.erase(list_iter);	
		}
	}
	...
	for(str_rev_iter=keyword.begin();str_rev_iter!=keyword.end();str_rev_iter++)
	{
		list_iter=find(alphabet.begin(), alphabet.end(), keyword[i]);
		if(list_iter!=alphabet.end())
		{
			alphabet.push_front(list_iter);	
		}
	}

 ...
 * huffman.cpp
 *
 * Your Name:jonathan rabolli
 * Your Collaborators:willuiam holden,eric lindenman
 *
...
void bitstring_helper(string from_map, map<char,string>& map_ref,freq_info* huffman_root){

if(huffman_root->is_leaf==true){
map_ref.insert(std::pair<char, string>(huffman_root->symbol, from_map));
}


else{
	from_map.append(".");
	bitstring_helper( from_map, map_ref,huffman_root->left);
	from_map.erase(from_map.size()-1);
	from_map.append("^");
	bitstring_helper( from_map, map_ref,huffman_root->right);
	from_map.erase(from_map.size()-1);
}
}
freq_info* build_huffman_tree(tree_queue &queue) {
 ...
 while(queue.size() > 1)
 {
	 freq_info* temp1 =queue.top();
	 queue.pop();
	 freq_info* temp2=queue.top();
	 queue.pop();
	 queue.push(init_freq_info_supernode(temp1,temp2));
	 }
	 root=queue.top();
  return root;
}
...
map<char, string> build_encoding_map(freq_info*& huffman_root) {
  ...
  if(huffman_root==NULL){
    ...
    string init_str;
    init_str.empty();
    bitstring_helper(init_str, ret,huffman_root);
...
string encode(string& original, map<char, string>& encoding_map) {
  ...
  for(i = 0; i < original.size(); i++)
    {
      swap = original[i];
      result = result + encoding_map[swap];
    }

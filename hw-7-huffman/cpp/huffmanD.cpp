...
freq_info* build_huffman_tree(tree_queue &queue)
{
  ...
  // implement me. set 'root' to something useful and return it.
	...
	while(queue.size() > 1)
	{
		...
		queue.push(init_freq_info_supernode(left,right));
...
map<char, string> build_encoding_map(freq_info*& huffman_root) 
{
  ...
  // see how to add and erase characters to/from the end of a string.
	//check if null
	//if leaf add to map
	//insert into encoding map
	//else (until i get to a leaf)
	//dont forget pointer to root
	freq_info* huff = huffman_root;
	if(huff != NULL)
	{
			if(huff->is_leaf == true)
				ret.insert(pair<char,string>(huff->symbol, path));
			else
			{
				path.push_back('.');
				build_encoding_map(huff->left);
				path.erase(path.end()-1);
				
				path.push_back('^');
				build_encoding_map(huff->right);
				path.erase(path.end()-1);
			}
	}
	return ret;
...
string encode(string& original, map<char, string>& encoding_map) 
{
	...
	{	
		result = result + encoding_map[original[i]];
